...
const SUBSCRIPTION_OFFER_ID = "saas-co-offer-1";
const SUBSCRIPTION_PROSPECTION_HOURS = 20;
...
export function buildSubscriptionScenarioActions(monthIndex: number, state: GameState): MonthActions {
  ...
  const offerActions: OfferAction[] = isCreationMonth
    ? [
        {
          kind: "create",
          spec: { id: SUBSCRIPTION_OFFER_ID, name: "Abonnement Subscription Co", businessModel: "service-hours", positioning: "standard", targetSegment: "", price: 29 },
        },
        { kind: "launch", offerId: SUBSCRIPTION_OFFER_ID },
      ]
    : [];
  ...
  const businessAction: BusinessAction = {
    ...
    founderProspectionHoursAllocated: SUBSCRIPTION_PROSPECTION_HOURS,
  ...
  return {
    timeAllocation: { emploi: 0, apprentissage: 20, business: 120 + SUBSCRIPTION_PROSPECTION_HOURS, reseau: 0 },
    jobHourlyWage: null,
